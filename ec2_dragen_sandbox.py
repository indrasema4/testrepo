...
account_id='417302553802'
image_id='ami-0be2609ba883822ec'
instance_type='t2.micro'
security_group_ids=['sg-03ab5eada1179b1b5']
subnet_id='subnet-0be094fc238ed8348'
key_name='aws-eb'
tag_name='dragen_ec2'
...
def find_ec2():
    ...
    for resp_inst in resp:
        ...
        instance_list.append( [resp_item['InstanceId'],tag_attached,resp_item['State']['Name']])
...
def action_ec2(action='nothing'):
    ...
    #tag_attached=[ (itm['Key'],itm['Value']) for  itm in  ec2_term['Tags'] if itm['Key'] == tag_name ]
        if action == 'terminate':
            ...
            yn=input("type YES to shutdown ")
            if yn == 'YES':
                if inst[2] != 'running':
                    print("this instance " + inst[0] + " is not in running state, stop is  not performed")
                    exit(0)
